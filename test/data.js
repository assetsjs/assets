...
test("w/o options", (t) =>
  resolveData("test/fixtures/duplicate-1.jpg").then((resolvedDataUrl) => {
...
test("discard query + preserve hash", (t) =>
  resolveData("test/fixtures/duplicate-1.jpg?foo=bar#hash").then(
    (resolvedDataUrl) => {
      t.is(resolvedDataUrl.slice(0, 32), "data:image/jpeg;base64,/9j/4AAQS");
...
test("svg", (t) =>
  resolveData("test/fixtures/images/vector.svg").then((resolvedDataUrl) => {
...
test("non-existing file", (t) =>
  resolveData("non-existing.gif").then(t.fail, (err) => {

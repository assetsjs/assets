...
test('w/o options', (t) => resolveUrl('test/fixtures/duplicate-1.jpg').then((resolvedUrl) => {
...
test('non-existing file', (t) => resolveUrl('non-existing.gif').then(t.fail, (err) => {
...
test('query + hash', (t) => resolveUrl('test/fixtures/images/picture.png?foo=bar&baz#hash').then(
  (resolvedUrl) => {
...
test('URI-encoded needle', (t) => resolveUrl('test/fixtures/white%20space.txt').then((resolvedUrl) => {
